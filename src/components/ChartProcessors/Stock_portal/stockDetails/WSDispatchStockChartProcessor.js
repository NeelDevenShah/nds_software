...
import WDispatchData from '../../../Chart_Datas/Stock_portal_Datas/stockDetails/WSDispatchData'
//For the chart Written thing we have imported the file in the one chart file

function WarehousesChartProcessor() {
    const [data, useData]=useState({
        labels: WDispatchData.map((data)=>data.type),
        datasets:[
            {
                label:"Warehouse Stock Details",
                data: WDispatchData.map((data)=>data.quantity),
                backgroundColor: [
                    "#17cf42",
                    "#2a71d0",
                    "#db1d1d"
    ...
    return (
        ...
        <div style={{width: 300}}>
          ...
          <p><strong>Total Planned Dispatch Of Stock</strong></p>
          </div>
        </div>
      </div>
  )
